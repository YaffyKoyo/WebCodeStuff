...
if (Meteor.isServer){
	Meteor.startup(function(){
		...
		//for (var i=0;i<1; i++){
		 	...
		 	var filename = 'jsonfiles/crime.json';
		 	...
		 	try{
		 		...
		 		rhythm_keys = Object.keys(song);
      			for (var j=0;j<rhythm_keys.length;j++){
      				console.log("type of "+rhythm_keys[j]+" is "+typeof(song[rhythm_keys[j]]));
      				// only use features that are numbers ... ignore arrays etc. 
      				if (typeof(song[rhythm_keys[j]]) === "number"){
      					single_features[rhythm_keys[j]] = song[rhythm_keys[j]];
      				}
      				if (typeof(song[rhythm_keys[j]]) === "object" && 
      					song[rhythm_keys[j]].length != undefined){// its an array
      					array_features[rhythm_keys[j]] = song[rhythm_keys[j]];
      				}
      				if (typeof(song[rhythm_keys[j]]) === "string"){
      					string_features[rhythm_keys[j]] = song[rhythm_keys[j]];
      				}
